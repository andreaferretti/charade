...
def parse_options():
    ...
    parser.add_argument('--data-dir', required=True, help='the directory that contains the data folders')
    parser.add_argument('--data-name', required=True, help='name of the folder containing dataset')
    parser.add_argument('--data', required=True, help='the CSV file that contains the training sentences')
    parser.add_argument('--num-labels', type=int, required=True, help='the number of labels')
    parser.add_argument('--sentence-column', type=str, required=True, help='the column of the data containing sentences')
...
if __name__ == '__main__':
    ...
    model = bert_for_sentence_classification.BertForSentenceClassification(options.lang, options.num_labels,
                                                                           options.pretrained_model_name_or_path,
                                                                           options.alpha, options.batch_size,
                                                                           options.num_epochs, options.random_state,
                                                                           options.max_len, options.do_lower_case)

    configs = {constants.DATA_FOLDER: os.path.join(options.data_dir, options.data_name),
               constants.TRAIN: options.data,
               constants.SENTENCE_COLUMN: options.sentence_column,
    ...
    training_info = model.train(dataset_loader, dataset_name=options.data_name, metrics=metrics, evaluation_phases=evaluation_phases)
