...
@DatasetReader.register("wikiner")
class WikinerDatasetReader(DatasetReader):
    """
    Reads instances from a pretokenised file where each line is a space
    separated sequence of tokens, each having the following format:

    WORD|POS-TAG|NER-TAG

    with a blank line indicating the end of each article,
    ...
    each sentence as an independent ``Instance``. (Technically the reader splits
    sentences on newlines, ignoring blank lines; in particular, it does the right
    ...
    tag_label: ``str``, optional (default=``ner``)
        Specify `ner` or `pos` to have that tag loaded into the instance field `tag`.
    feature_labels: ``Sequence[str]``, optional (default=``()``)
        These labels will be loaded as features into the corresponding instance fields:
        ``pos`` -> ``pos_tags``, ``ner`` -> ``ner_tags``
        Each will have its own namespace: ``pos_tags``, ``ner_tags``.
        If you want to use one of the tags as a `feature` in your model, it should be
        specified here.
    coding_scheme: ``str``, optional (default=``IOB1``)
        Specifies the coding scheme for ``ner_labels``.
        Valid options are ``IOB1`` and ``BIOUL``.  The ``IOB1`` default maintains
        the original IOB1 scheme in the aij-wikiner data.
    ...
    def _read(self, file_path: str) -> Iterable[Instance]:
        ...
        with open(file_path, "r") as data_file:
            ...
            for line in data_file.readlines():
                ...
                if line != '':
                    tokens = line.split()
                    fields = [token.split('|') for token in line.split()]
                    # unzipping trick returns tuples, but our Fields need lists
                    fields = [list(field) for field in zip(*fields)]
                    tokens_, pos_tags, ner_tags = fields
                    tokens = [Token(token) for token in tokens_]

                    yield self.text_to_instance(tokens, pos_tags, ner_tags)
