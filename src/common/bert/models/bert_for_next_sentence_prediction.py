...
class BertForNextSentencePrediction(bert_model.BertModel):
    """
    Bert model for next sentence prediction task.
    ...
    Moreover, the config file also specifies the columns where first  and
    second sentences and the target are stored: respectively in the fields
    {constants.FIRST_SENTENCE_COLUMN), {constants.SECOND_SENTENCE_COLUMN) and
    ...
    tokenizer : transformers.tokenization_bert.BertTokenizer
        Tokenizer
    model : transformers.modeling_bert.BertForNextSentencePrediction
    ...
    def __init__(self, language, pretrained_model_name_or_path='bert-base-uncased', alpha=1e-5, batch_size=32,
                 epochs=4, random_state=0, max_len = None, do_lower_case=True):

        super().__init__(language, False, pretrained_model_name_or_path, alpha, batch_size, epochs, random_state, max_len, do_lower_case)
    ...
    def _get_model(self):
        ...
        """
        model = transformers.BertForNextSentencePrediction.from_pretrained(self.pretrained_model_name_or_path,
    ...
    def _get_tensor_dataset(data, target_column):
        ...
        token_type_ids = torch.tensor(data[constants.TOKEN_TYPE_IDS].tolist())

        y = None
        if target_column in data.columns:
            y = torch.tensor(data[target_column].tolist())

        tensor_dataset = torch.utils.data.TensorDataset(input_ids, mask, token_type_ids, y) if y is not None else torch.utils.data.TensorDataset(input_ids, mask)
    ...
    def _evaluate(self, logits, labels, metric):
        ...
        metric_result = models_utilities.evaluate(y, y_pred, metric, 'binary')

        return metric_result

    def predict_example(self, sentence1, sentence2):

        # data preprocessing
        data = pd.DataFrame({'sentence1': [sentence1], 'sentence2' : [sentence2]})
        dataset_preprocessing.preprocess_dataset_sentences_pair(data, 'sentence1', 'sentence2', self.max_len, self.tokenizer, self.language)
        ...
        token_type_ids = torch.tensor(data[constants.TOKEN_TYPE_IDS].tolist()).to(self.device)

        # forward propagation
        with torch.no_grad():
            outputs = self.model(input_ids, token_type_ids=token_type_ids, attention_mask=mask)
